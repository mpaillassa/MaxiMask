...
class MaxiTrack_inference(object):
    ...
    def process_file(self, file_name, tf_model):
        ...
        if self.frac!=1:
            ...
            if nb_hdus_to_use==0:
                nb_hdus_to_use = 1
            log.info(f"Using fraction option with {self.frac} fraction of {nb_hdus} HDUs")
        ...
        if "[" in im_path_ext:
            ...
            with fits.open(file_name.split("[")[0]) as file_hdu:
                ...
                if check:
                    ...
                    log.info(f"Ignoring HDU {spec_hdu_idx} because not adequate data format")
        else:
...
def main():
    ...
    frac = args.frac
    if frac<=0 or frac>1:
        raise ValueError(f"Provided fraction parameter is {frac} but should be between 0 and 1")
