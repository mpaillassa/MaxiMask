...
def get_file_list(im_path):
    ...
    if os.path.isdir(im_path):
        ...
        if im_path_ext==".list":
            ...
            for line in lines:
                fname = line.rstrip()
                if (os.path.isfile(fname) or "[" in fname) and "fits" in fname and "mask" not in fname:
                    # fits file or specific fits file hdu
                    file_list.append(fname)
        elif im_path_ext==".fits" or (im_path_ext in [".fz", ".gz"] and ".fits" in im_path):
            # im_path is a fits file
            file_list.append(im_path)
        else:
            raise ValueError(f"Provided im_path <{im_path}> does not have a valid format")
...
def check_hdu(hdu, min_size):
    ...
    dt = infos[5]
    data_type_b = "float16" in dt or "float32" in dt or "float64" in dt or "int16" in dt or "int32" in dt or "int64" in dt or "uint32" in dt
...
def build_map(grid_v, h_blocks, w_blocks, x_l, y_l, mesh_size, h, w):
    ...
    f = interp.RectBivariateSpline(
        np.arange(-mesh_size / 2, (h_blocks + 1) * mesh_size, mesh_size) + (h % mesh_size) // 2,
        np.arange(-mesh_size / 2, (w_blocks + 1) * mesh_size, mesh_size) + (w % mesh_size) // 2,
        pad_grid,
    )
...
def clean_grid(grid):
    ...
    while True:
        ...
        out_idx = np.where(np.logical_or(cur_mask * (grid > cur_med+3*cur_std), cur_mask * (grid < cur_med-3*cur_std)))
            ...
            for k in range(len(idx[0])):
                ...
                for yy in range(y-1, y+2):
                    for xx in range(x-1, x+2):
                        if yy>=0 and yy<h and xx>=0 and xx<w and not math.isnan(grid[yy, xx]):
                            tmp_mask[yy, xx] = 1
